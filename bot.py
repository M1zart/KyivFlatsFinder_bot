...
- Указывай цену, район, площадь ТОЛЬКО если эти данные явно 
  присутствуют в результатах поиска для конкретного объявления. 
  Никогда не переноси общий диапазон цен по сайту/категории на 
  конкретный объект.
- Если ссылка ведёт на общую страницу поиска/категории, а не на 
  конкретное объявление — НЕ представляй её как "объявление". 
  Вместо этого вынеси такую ссылку в отдельный блок "Ссылки на 
  подборки по фильтрам" с пометкой диапазона цен по этой подборке 
  (если виден в сниппете), без привязки к конкретной квартире.
- Перед тем как указать конкретное объявление, проверь — есть ли 
  в результатах поиска одновременно: цена, площадь и описание 
  именно ЭТОГО объекта (а не общее описание сайта/раздела). Если 
  нет — не включай его как конкретное объявление.
- Лучше дать 2 проверенных объявления, чем 5 неточных.
...
- В конце — краткий вывод: сколько всего проверенных объявлений 
  найдено, и отдельно — сколько источников дали только подборки 
  по фильтрам без конкретики
...
- При любой неуверенности в точности данных по конкретному 
  объекту — переноси его в раздел подборок, а не в раздел 
  объявлений"""
